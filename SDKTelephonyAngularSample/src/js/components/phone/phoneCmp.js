angular.module('sample').component('rbxPhone', {
    ...
    controller : function rbcPhoneCtrl (rainbowSDK, $rootScope, $scope, Notification) {
        ...
        $scope.call = function call() {
            ...
            rainbowSDK.telephony.callByNumber($scope.phoneNumber).then(function(data) {
            }).catch(function(err) {
        ...
        var onTelephonyStateChanged = function onTelephonyStateChanged() {
            ...
            $scope.phoneCmpAgentVersion = $scope.agentVersion === "unknown" ? "phoneCmp-agent-color-warning" : "phoneCmp-agent-color-ok";
            $scope.phoneCmpAgentXmpp = $scope.agentXmpp === "unknown" ? "phoneCmp-agent-color-warning" : $scope.agentXmpp === "stopped" ? "phoneCmp-agent-color-error" : "phoneCmp-agent-color-ok";
            $scope.phoneCmpAgentPhone = $scope.phoneAPI === "unknown" ? "phoneCmp-agent-color-warning" : $scope.phoneAPI === "disconnected" ? "phoneCmp-agent-color-error" : "phoneCmp-agent-color-ok";
            ...
            if(rainbowSDK.telephony.isTelephonyAvailable()) {
                ...
                if(monitoredPhone) {
                    ...
                    Notification.warning({
                        title: "Telephony Service",
                        message: "There is no information on the associated phone and PBX system",
                        positionY: 'bottom'
                    });
                }
            }
        ...
        var onTelephonyCallChanged = function onTelephonyCallChanged(event, call) {
            
            if(call.id in $scope.calls) {

                console.log("DEMO", "Call " + call.id + " --> " + call.status.value);

                if(call.status.value === rainbowSDK.telephony.RAINBOW_TELEPHONYUNKNOW) {
                    $scope.calls[call.id] = null;
                    delete $scope.calls[call.id];
                    console.log("DEMO", "Call " + call.id + ": " + call.status.value + " (removed)");
                }
            }
            else {
                // don't add call in case of releasing or Unknown
                if(call.status.value !== rainbowSDK.telephony.RAINBOW_TELEPHONYUNKNOW) {
                    console.log("DEMO", "Call " + call.id + ": " + call.status.value + " (added)");
                    $scope.calls[call.id] = call;
                }
                else {
                    console.log("DEMO", "Call " + call.id + ": " + call.status.value + " (already removed)");
                }
            }
        ...
        this.$onInit = function () {
            ...
            handlers.push($rootScope.$on(rainbowSDK.telephony.RAINBOW_ONTELEPHONYSTARTED, onTelephonyStarted));
            handlers.push($rootScope.$on(rainbowSDK.telephony.RAINBOW_ONTELEPHONYSTOPPED, onTelephonyStopped));
            handlers.push($rootScope.$on(rainbowSDK.telephony.RAINBOW_ONTELEPHONYCALLSTATECHANGED, onTelephonyCallChanged));
        };
