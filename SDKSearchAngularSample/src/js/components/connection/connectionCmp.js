    ...
    controller : function rbcConnectionCtrl(rainbowSDK, $rootScope, $scope) {
        ...
        $scope.signin = function() {
            ...
            switch ($scope.selectedItem.value) {
                case "rainbow":
                    rainbowSDK.connection.signinOnRainbowOfficial($scope.user.name, $scope.user.password).then(function(__account) {
                ...
                break;
                default:
                    rainbowSDK.connection.signin($scope.user.name, $scope.user.password).then(function(__account) {
                        console.log("[DEMO] :: Successfully signed!");
                        $scope.isLoading = false;
                        $scope.isConnected = true;
                    }).catch(function(err) {
